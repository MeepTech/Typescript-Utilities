...
HookConstructor.SYMBOL = $HOOK;

HookConstructor.is = isHook as any;
 ...
 */
type OnHookChanged<T> = HookListener<T> &
  Readonly<{
    /**
 ...
 * Creates a new hook that can be used to reference a value here and elsewhere.
 */
export function hook<T>(value: Hook<T>): Hook<T>;

/**
 * Creates a new hook that can be used to reference a value here and elsewhere.
 */
export function hook<T>(value: T): Hook<T>;
...
export function hook<T>(value?: T): Hook<T> {
  const props: Hook<T>[typeof $HOOK] =
    new HookProps(
      value
    ) as unknown as Hook<T>[typeof $HOOK];
